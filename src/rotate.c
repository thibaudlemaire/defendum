...
int rotate_left_position;
int rotate_right_position;
int rotate_front_position;
...
int init_rotate( void )
{
    ...
    while( !(tacho_get_state(MOTOR_ROTATE) && TACHO_STALLED))
        sleep_ms(ROTATE_PERIOD);
    rotate_left_position = tacho_get_position(MOTOR_ROTATE, 0);

    tacho_set_speed_sp(MOTOR_ROTATE, -ROTATE_SPEED);
    tacho_run_forever( MOTOR_ROTATE );
    while( !(tacho_get_state(MOTOR_ROTATE) && TACHO_STALLED))
        sleep_ms(ROTATE_PERIOD);
    rotate_right_position = tacho_get_position(MOTOR_ROTATE, 0);
